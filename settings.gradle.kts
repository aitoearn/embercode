...
extensions.configure<expo.modules.plugin.ExpoAutolinkingSettingsExtension> {
    ...
    // @getpaseo/app 传递依赖会带入下列 Expo 开发/更新模块；PhoneCode 嵌入式宿主不需要它们：
    // - expo-updates：无 android/ 子目录时 projectRoot 反算错误，构建失败
    // - expo-dev-client / launcher / menu：会劫持 RemoteRnActivity → DevLauncherActivity，白屏
    // exclude 以空格拼接传给 CLI，可同时排除多个包名。
    exclude = listOf(
        "expo-updates",
        "expo-dev-client",
        "expo-dev-launcher",
